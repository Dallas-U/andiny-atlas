...
# Sprint 24 – Investigation Audit Trail

---

## Executive Summary

Sprint 24 introduced an immutable audit trail for investigation updates.

Prior to this sprint, investigation cases could be updated, but the previous state of an investigation was not preserved. Once an update was applied, earlier values for status, reason, and next action were overwritten.

Sprint 24 solved this limitation by introducing immutable audit-history records that capture the state of an investigation immediately before every update. Each update now executes as a single transaction that records the previous state and applies the new values together.

The implementation preserves the project's layered architecture:
...
Introduce a reliable, immutable audit history for investigation updates while maintaining clean architectural boundaries, transactional consistency, and complete automated test coverage.

---

## Engineering Motivation

Before Sprint 24, investigations could be edited, but there was no permanent record of earlier values.

This presented several challenges:

- Previous investigation decisions were lost.
- Supervisors could not review how a case evolved.
- Operational reporting lacked historical context.
- There was no audit evidence for changes.
- Future compliance features would require redesign.

An enterprise support platform should preserve operational history rather than replacing it.

Sprint 24 addressed this by introducing immutable audit records that represent the investigation state immediately before every update.

---

## Architectural Decision

The central design decision was to model audit history as its own domain entity instead of embedding historical data within the investigation itself.

The new domain object represents one historical snapshot:
...
Separating audit history from the investigation entity keeps the current investigation focused on present state while preserving historical information independently.

---

## Architecture Before Sprint 24

Investigation updates followed this path:
...
Once the update completed, the previous investigation state no longer existed.

---

## Architecture After Sprint 24

Sprint 24 expanded the update workflow:
...
History retrieval follows a separate read path:

```text
GET /support/cases/{case_id}/history
    │
    ▼
CaseManager
    │
...
```text
backend/app/domain/case_history.py
backend/app/models/history_response.py
backend/tests/test_case_history_repository.py
backend/tests/test_case_history_service.py
backend/tests/test_case_history_api.py
```
...
```text
backend/app/database/mappers.py
...
The repository also introduced:

```python
get_case_history(case_id)
```

which returns history ordered chronologically.

---

## Service-Layer Integration

The service layer now automatically creates audit entries.
...
Validate ownership
    │
    ▼
Create immutable CaseHistory
    │
    ▼
Repository transaction
```

The API no longer needs to understand audit creation.
...
The endpoint returns the complete audit history for an investigation.
...
The endpoint uses the same authorization and standardized error handling already established for investigation retrieval.

---

## Transaction Integrity

A key engineering objective was ensuring that history insertion and investigation updates occur atomically.
...
Sprint 24 introduced dedicated automated tests covering the following areas.
...
- Ownership validation
- Pre-update snapshot correctness
- Missing investigation handling
...
The complete automated suite increased to:
...
- Transactional update consistency.
- Dedicated audit domain model.
- Repository-managed transactions.
- Service-owned audit creation.
- Separate history response DTO.
...
- Audit history should be modelled independently of current state.
- Repository transactions are the correct boundary for atomic persistence.
- Services should own business events such as history creation.
- Immutable history simplifies future reporting and compliance.
- Dedicated response models improve API clarity.
- Separate test suites make long-term maintenance easier.

---

## Why Sprint 24 Matters

Sprint 24 transforms investigation updates from destructive edits into traceable business events.

The platform can now answer questions such as:

- What was the investigation status before this change?
- Who performed the update?
- When was the investigation modified?
...
The most important outcome of Sprint 24 is not the new endpoint.

It is the establishment of immutable business history as a first-class architectural concept.

Current investigation state and historical investigation state are now represented independently, allowing future reporting, analytics, and compliance capabilities to build on a stable foundation.
...
At the completion of Sprint 24, Andiny Atlas includes:

- Clean Architecture.
- Domain-driven investigation model.
- SQLAlchemy persistence.
- Repository pattern.
- JWT authentication.
- Ownership-aware investigations.
- Enterprise querying.
- SQL-level filtering.
- SQL-level sorting.
- SQL pagination.
- Immutable investigation audit history.
- Transactional update workflow.
- Dedicated history endpoint.
- Dedicated audit response models.
...
When creating the Sprint 24 release, update this section with the actual Git information.

**Commit Message**

```text
feat(sprint-24): implement immutable investigation audit trail
```
...
## Sprint 25 — Security, Authorization, and Audit Architecture
...
Sprint 25 marked one of the most significant architectural milestones in the development of **Andiny Atlas**.

Previous sprints had already established the foundation for Clean Architecture, separated the Domain layer from infrastructure concerns, introduced immutable domain entities, and completed the migration from ORM-centric repositories to domain-centric repositories.

Although the system had become structurally sound, one critical concern remained unresolved:

> **Security had not yet matured beyond authentication.**

Authentication already allowed the application to identify users, but authorization remained too simplistic. Any authenticated user could potentially reach functionality that should have been restricted according to organisational responsibility, ownership, or operational authority.

Sprint 25 transformed the existing authentication implementation into a complete authorization architecture while preserving the project's Clean Architecture principles.

The sprint also introduced a comprehensive case audit-history system, ensuring that every significant modification made to a support case can be traced permanently.

---

## Sprint Objectives

Sprint 25 focused on five primary objectives:

1. Introduce hierarchical **Role-Based Access Control (RBAC)**.
2. Eliminate scattered authorization logic from endpoint implementations.
3. Introduce capability-based authorization dependencies.
4. Create immutable audit history for case modifications.
5. Maintain strict architectural isolation between the Domain and Infrastructure layers.

Every implementation decision made during the sprint supported one or more of these objectives.

---

## Architectural Goals

Rather than implementing security through repeated conditional checks inside API endpoints, the application adopted a layered authorization architecture.
...
- the API layer remains focused on HTTP concerns;
- authorization rules remain centralized;
- the service layer remains focused on business rules;
- repositories remain responsible for persistence;
- the Domain layer remains independent of frameworks.

---

## User Domain Model

Sprint 25 introduced a dedicated `User` domain entity.
...
Although this appeared to be a small change, it had major architectural consequences.
...
Every authenticated request now operates on a real domain user loaded from persistence rather than relying exclusively on data embedded inside a token.

This allows authorization decisions to reflect the user's current account status and role.

---

## JWT Simplification

Earlier implementations stored multiple user attributes inside JWT claims.

Sprint 25 intentionally simplified the token so that it contains only the user identifier in the `sub` claim.

```text
sub
```

The subject value uniquely identifies the authenticated user.

Each authenticated request now follows this sequence:

1. Decode the JWT.
2. Extract the `sub` claim.
3. Load the user from the repository.
4. Validate that the account exists and is active.
5. Apply the required authorization rule.

### Benefits

- Role changes take effect immediately.
- User deactivation takes effect immediately.
- Tokens remain lightweight.
- Authorization data cannot become stale inside long-lived tokens.
- Security decisions remain grounded in the current database state.

---

## Role Hierarchy

Sprint 25 introduced the application's first hierarchical authorization model.
...
- A **Supervisor** can perform every action available to an Agent.
- An **Administrator** can perform every action available to a Supervisor.
- A **Super Administrator** can perform every authorized action in the system.

This structure significantly reduces duplicated permission logic and makes future security rules easier to extend.

---

## Capability-Based Authorization

Rather than checking role names directly throughout the application, authorization is expressed in terms of capabilities.
...
Endpoints declare the capability they require instead of implementing role comparisons themselves.
...
- Endpoint definitions remain readable.
- Authorization rules remain centralized.
- Role-to-capability mappings can evolve without rewriting endpoints.
- Future permission models can become more granular.
...
Sprint 25 established explicit authorization requirements for support endpoints.

| Endpoint | Minimum Role | Additional Rule |
|---|---:|---|
| `POST /support/investigate` | Agent | None |
| `GET /support/my-cases` | Agent | Returns only the authenticated user's cases |
| `GET /support/cases` | Supervisor | None |
| `GET /support/cases/{id}` | Supervisor | None |
| `GET /support/cases/{id}/history` | Supervisor | None |
| `PATCH /support/cases/{id}` | Agent | User must own the case |
| `GET /support/statistics` | Supervisor | None |

Role authorization is enforced through dependency injection.

Ownership rules remain enforced in the service layer because ownership is a business rule rather than a transport concern.

---

## Audit History Architecture

One of the largest functional additions in Sprint 25 was complete case audit history.

Whenever a case is modified, the application preserves the pre-update state before applying the new values.
...
The snapshot represents the case as it existed immediately before the modification.

This guarantees that previous values are not lost.

Each audit-history record preserves information such as:
...
The resulting history forms a permanent chronological record of case evolution.

---

## Repository Evolution

Sprint 25 clarified the responsibilities of the case and history repositories.

### Case Repository

The case repository is responsible for:
...
The history repository is responsible for:

- creating pre-update snapshots;
- retrieving case history;
- returning history in chronological order;
- preserving immutable audit records.

This separation follows the **Single Responsibility Principle** and avoids mixing current-state persistence with historical-state persistence.

---

## Clean Architecture Validation

Sprint 25 verified that every architectural layer continued to respect its boundaries.
...
It contains no direct dependency on:
...
Services operate exclusively on domain entities and repository abstractions.

No ORM models are used inside business logic.

### Repository Layer

Repositories translate between persistence models and domain entities.
...
Dedicated mapper functions isolate the conversion logic.
...
Business rules do not leak into endpoint implementations.

---

## Testing Strategy

Sprint 25 significantly expanded the automated test suite.
...
- ownership enforcement;
- unauthorized cross-user updates.
...
Sprint 25 produced one of the project's most instructive debugging exercises.

After hierarchical RBAC was introduced, fifteen tests initially failed with authorization errors.

### Initial Assumption

The authorization implementation itself appeared to be defective.

### Actual Causes

The failures were caused by test infrastructure rather than production authorization logic:

1. Supervisor-only endpoint tests were still using an Agent-authenticated client.
2. A new `supervisor_client` fixture was required.
3. The Supervisor fixture needed to construct the new domain `User`.
4. The `User` entity required the `is_active` field.
5. The fixture initially omitted that required argument.

### Resolution Process

The debugging process followed a disciplined sequence:

1. Identify the common failure pattern.
2. Inspect the role attached to the current test user.
3. Verify endpoint capability requirements.
4. Introduce a Supervisor-authenticated client fixture.
5. Update Supervisor-only tests to use the correct fixture.
6. Inspect the exact `User` constructor.
7. Add `is_active=True`.
8. Run the complete suite without cache.
...
> **Investigate failing assumptions before modifying production code.**

The security implementation was correct. The tests had to be updated to represent the new authorization model accurately.

---

## Sprint 25 Timeline

The sprint progressed through a series of connected architectural milestones.

| Phase | Focus | Outcome |
|---|---|---|
| **25.1 — Domain Security Foundation** | Introduced the `User` domain entity and aligned authentication services with domain objects. | Authentication no longer depended directly on ORM entities. |
| **25.2 — Persistence and Mapping Alignment** | Updated user repositories, database models, and mappers. | ORM-to-domain conversion became explicit and testable. |
| **25.3 — JWT and Current-User Refactor** | Reduced JWT claims to `sub` and loaded the current user from persistence on every request. | Roles and account status became current at request time. |
| **25.4 — Hierarchical RBAC** | Added four user roles, role hierarchy, and capability dependencies. | Endpoint access became centralized and declarative. |
| **25.5 — Case Ownership and Protected Operations** | Enforced Agent ownership for edits and Supervisor access for global views, history, and statistics. | Authorization aligned with operational responsibilities. |
| **25.6 — Immutable Audit History** | Added the `CaseHistory` domain entity, history persistence, chronological retrieval, and API exposure. | Every case modification became traceable. |
| **25.7 — Test Migration and Debugging** | Added Supervisor fixtures, updated protected endpoint tests, and corrected the `is_active` fixture requirement. | The full suite returned to green. |
| **25.8 — Sprint Validation** | Ran the complete automated suite and reviewed architectural boundaries. | Sprint 25 closed with **85/85 passing tests**. |
| **25.9 — Documentation and Closure** | Updated the Engineering Handbook, recorded lessons learned, and prepared the sprint milestone for tagging. | The technical and historical record of Sprint 25 was completed. |
...
Domain User
    │
    ▼
Repository and Mapper Alignment
    │
    ▼
Database-Backed Current User
    │
    ▼
JWT Simplification
    │
...
Authorization logic should not be repeated across endpoint implementations.

Centralized capability dependencies make rules easier to inspect, test, and change.

### Domain Models Should Remain Framework-Independent

Keeping FastAPI, SQLAlchemy, and Pydantic outside the Domain layer significantly reduces coupling and protects business logic from infrastructure changes.

### Ownership Is a Business Rule

Role checks belong in authorization dependencies, but ownership checks belong in services because ownership depends on business context.

### Tests Validate Architecture

The test suite did more than validate outputs. It exposed assumptions about role identity, fixture construction, and domain-model evolution.

### Audit History Must Be Immutable

Historical records must describe what happened at a point in time. They must not change when the current case changes.

### Test Fixtures Must Evolve With the Domain

When domain constructors change, fixtures must be updated explicitly. Test objects should reflect the same invariants required by production code.
...
| Domain Models | 5 |
| User Roles | 4 |
| Authorization Capabilities | 8 |
| Protected Support Endpoints | 7 |
| History Repository | Completed |
| Immutable Audit Trail | Completed |
| Hierarchical RBAC | Completed |
...
Sprint 25 successfully transformed Andiny Atlas from an authenticated application into a properly authorized and auditable system.
...
- immediate role-change enforcement;
- ownership-aware case editing;
- immutable case audit history;
- comprehensive automated testing;
- verified Clean Architecture boundaries.

The sprint concluded with a completely passing test suite and a stable codebase ready for future enterprise capabilities.

---

## Looking Ahead

Sprint 26 will build on the security foundation established during Sprint 25.

Rather than revisiting foundational architecture, the next sprint should focus on enterprise operational maturity, including areas such as:
...
The objective of Sprint 26 should be to operationalize the secure foundation created in Sprint 25.

---

## Sprint 25 Summary

Sprint 25 was not merely a security-feature sprint.

It established a security architecture that:

- aligns with Clean Architecture;
- remains maintainable as the system grows;
- supports organisational responsibilities;
- protects sensitive operations;
- preserves a permanent audit trail;
- is validated by a comprehensive automated test suite.

By the end of Sprint 25, Andiny Atlas had evolved from a well-structured authenticated application into a secure, role-aware, ownership-aware, and auditable enterprise platform.
...
**Sprint:** 25  
**Title:** Security, Authorization, and Audit Architecture  
**Status:** Complete  
**Test Result:** 85 passed, 0 failed, 0 errors  
**Closing Commit:** `306d028 feat(auth): implement hierarchical RBAC`  
**Recommended Tag:** `sprint-25-complete`

# Sprint 26 – Enterprise Operations & Administration (Phase 1)

**Sprint Status:** ✅ Completed *(Backend API v1 Stabilization)*

---

# Sprint Objective

The objective of Sprint 26 was to establish the operational and security foundation required for an enterprise-grade platform before transitioning to frontend development.
...
Unlike previous sprints, Sprint 26 emphasized validating the platform through end-to-end execution rather than simply implementing additional backend features.
...
The platform successfully authenticates registered users and issues signed JWT access tokens that are used to access protected resources.
...
Implemented hierarchical authorization roles:
...
Administrative endpoints are accessible only to authorized roles.

**Status:** ✅ Complete

---

## 3. User Administration Foundation

### Completed

Implemented the administrative API for retrieving users.
...
Successfully returned all seeded development users.

**Status:** ✅ Complete

---

## 4. Development User Seeder

A development seeding mechanism was introduced to support application testing.

### Development Accounts

| Role | Email Address |
|------|---------------|
| SUPER_ADMIN | superadmin@andiny.example.com |
| ADMIN | admin@andiny.example.com |
| SUPERVISOR | supervisor@andiny.example.com |
| AGENT | agent@andiny.example.com |
...
During Sprint 26, an Alembic migration inconsistency was discovered.

### Issue

The database revision history referenced an invalid placeholder revision, preventing Alembic from resolving the migration chain correctly.

### Resolution

- Corrected the migration revision
- Updated the Alembic version table
- Verified the migration head
...
Validated the complete authentication workflow.
...
Authentication successfully generated JWT access tokens and granted access to protected resources.

**Status:** ✅ Complete

---

## 8. Administrative Endpoint Validation
...
## 9. Operational CLI Foundation

Introduced the initial operational command-line framework.
...
Unlike previous sprints, Sprint 26 concluded with full functional validation of the complete security pipeline.
...
This confirmed that **Backend API v1** is operational and capable of supporting frontend development.
...
Resolved through manual repair of the Alembic revision table.
...
Resolved by creating and applying the missing migration for the **User Role** column.

---

## 3. Swagger OAuth2 Authorization

During testing, Swagger's **Authorize** button returned **HTTP 422 (Unprocessable Entity).**

### Root Cause

Swagger OAuth2 Password Flow submits credentials using:

```text
application/x-www-form-urlencoded
```

while the implemented login endpoint accepts:
...
Authentication was successfully validated through direct API requests.

### Planned Resolution

Introduce a dedicated OAuth2 token endpoint compatible with Swagger while preserving the JSON login endpoint for frontend clients.
...
Sprint 26 demonstrated the importance of validating implemented functionality through real execution rather than relying solely on successful builds or automated tests.

Testing the platform exposed several issues that would not have been discovered through implementation alone, including:

- Migration Inconsistencies
- Configuration Mismatches
- Swagger OAuth2 Integration Differences
- Development Tooling Improvements

Resolving these issues before frontend development significantly increased confidence in the stability of **Backend API v1**.

---

# Sprint Outcome

Sprint 26 successfully established the enterprise security and operational foundation of **Andiny Atlas**.
...
Backend API v1 has been validated and is considered sufficiently stable to support frontend design and implementation.

---

# Strategic Decision

Following the successful completion of Sprint 26, the project roadmap has been updated.

Rather than continuing to expand backend functionality, development will now transition to the frontend to validate the API through real-world application usage.

Any backend enhancements identified during frontend implementation will be addressed incrementally, ensuring that the platform evolves based on actual user workflows and application needs rather than assumptions.

This marks an important transition in the project—from **backend-first development** to **full-stack product validation**, where the frontend becomes the primary driver for refining the platform.

---

# Next Phase

# Sprint 27 – Frontend Architecture, Dashboard Design, and UI/UX Planning

Sprint 27 will focus on establishing the frontend foundation of Andiny Atlas through a structured product design process before implementation.

## Objectives

- Define the overall information architecture.
- Design the enterprise dashboard experience.
- Establish the application's navigation structure.
- Create the UI/UX blueprint.
- Develop the frontend architecture.
- Prepare the design system and reusable component library.
- Commence frontend implementation against the validated **Backend API v1**.

This transition represents the beginning of end-to-end product validation, where backend capabilities will be exercised through real user workflows, ensuring the platform evolves based on practical usage and enterprise requirements rather than theoretical assumptions.
